...
class sale_order_line(osv.osv):
    ...
    def layout_type_change(self, cr, uid, ids, layout_type):
        ...
        vals = {
            ...
            'price_unit': 0.0,
            'price_subtotal': 0.0,
            'quantity': 0 if layout_type != 'discount' else 1,
            'discount': 0.0,
            'invoice_line_tax_id': False,
            'account_analytic_id': False,
            'product_uom_qty': 0.0 if layout_type != 'discount' else 1.0,
        }
        vals['name'] = layout_val_2_text(layout_type)
        
        cr.execute("Select id from product_uom where name ilike 'unit%%' or name ilike '%%pce%%' and active is true order by id")
        row = cr.fetchone()
        if row and row[0]:
            vals.update({
                'product_uom': row[0],
...
class sale_order(osv.osv):
    ...
    def _total_discount(self, cr, uid, ids, name, arg, context=None):
        res = {}
        for obj in self.browse(cr, uid, ids, context=context):

            sub_total = 0.0
            for ln in obj.order_line:
                if ln.layout_type == 'discount':
                    sub_total += ln.price_subtotal
            
            res[obj.id] = sub_total
        return res

    _columns = {
        'total_discount': fields.function(_total_discount, string='Discount total', digits_compute= dp.get_precision('Account')),
    }
