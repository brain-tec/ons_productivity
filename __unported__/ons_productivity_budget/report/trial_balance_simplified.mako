    ...
    <body>
        ...
        <div class="act_as_table data_table">
            <div class="act_as_row labels">
                <div class="act_as_cell">${_('Chart of Account')} V2</div>
        ...
        <div class="act_as_table list_table" style="margin-top: 20px;">
            ...
            <div class="act_as_thead">
                <div class="act_as_row labels">
                    ...
                    %if comparison_mode == 'no_comparison' or budget_period_to != 0:
                    ...
                    %if comparison_mode == 'no_comparison' or not fiscalyear or budget_period_to != 0:
                        ${_('Balance')}
                    %else:
                    ...
                    %if comparison_mode in ('single', 'multiple'):
                        %for index in range(nb_comparison):
                            ...
                            %if comparison_mode == 'single' and budget_period_to == 0:  ## no diff in multiple comparisons because it shows too data
                                <div class="act_as_cell amount" style="width: 30px;">${_('Difference')}</div>
                                <div class="act_as_cell amount" style="width: 30px;">${_('% Difference')}</div>
                            %endif
                        %endfor
                    %endif
                    %if budget_period_to != 0:
                        <div class="act_as_cell amount" style="width: 30px;">${_('Ratio')}</div>
                        <div class="act_as_cell amount" style="width: 30px;">${_('Budget')}</div>
                    %endif
                </div>
            </div>
            ...
            <div class="act_as_tbody">
                ...
                %for current_account in objects:
                    ...
                    <div class="act_as_row lines ${level_class} ${"%s_account_type" % (current_account.type,)}">
                        ...
                        ## account name
                        <div class="act_as_cell" style="padding-left: ${level * 0}px;">${current_account.name}</div>
                        %if comparison_mode == 'no_comparison' or budget_period_to != 0:
                            %if initial_balance_mode:
                                ## opening balance
                        ...
                        %endif
                        %if budget_period_to != 0:
                            <div class="act_as_cell amount">${ get_budget_value(current_account, 'onsp_budget_rate', '-', 2, ' %') }</div>
                            <div class="act_as_cell amount">${ get_budget_value(current_account, 'onsp_budget_cum') and formatLang(get_budget_value(current_account, 'onsp_budget_cum')) or '-' | amount }</div>
                        %endif
                    </div>
                %endfor
            </div>
