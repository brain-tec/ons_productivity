    ...
    def price_get_multi(self, cr, uid, pricelist_ids, products_by_qty_by_partner, context=None):
        ...
        for product_id, qty, partner in products_by_qty_by_partner:
            for pricelist_id in pricelist_ids:
                ...
                ons_way = self.pool.get('ir.values').get_default(cr, uid, 'product.pricelist', 'onsp_sale_pricelist_vs_supplier')
                if ons_way:
                    partner_where = 'true'
                    partner_args = ()
                    product_pricelist = self.browse(cr, uid, pricelist_id, context=context)
                    if product_pricelist and product_pricelist.type == 'purchase' and partner:
