...
class account_invoice(models.Model):
    ...
    def _get_ordered_lines(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            query = """select soli.invoice_id
                        from sale_order_line_invoice_rel soli
                        left join sale_order_line sol on soli.order_line_id=sol.id
                        inner join sale_order so on so.id=sol.order_id
                        where soli.invoice_id in %s
                        order by so.name asc, soli.invoice_id desc
            """ % (str(tuple(ail_ids)), )
            cr.execute(query)
    ...
    def invoice_print(self):
        ...
        self.sent = True
        return self.env['report'].get_action(self, 'ons_productivity_invoicing.report_invoice_by_so')
